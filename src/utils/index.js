...
export const chunkFile = (param, total, hash, size) => {
	return Array.from(Array.from(Array(total).keys())).map((item, index) => {
		let start = index * size;
		let end = Math.min(param.file.size, start + size);
		const form = new FormData();
		// console.log(param.file);
		// form.append('file',param.file);
		form.append('file_name',param.file.name);
		form.append('blob_num',index+1);
		form.append('video', param.file.slice(start, end));
		return request({
			method: 'post',
			url: '/api/classroom/video/add',
			data: form,
			headers: {
