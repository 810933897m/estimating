        ...
        <!-- :before-close="handleClose" -->
            <div style="height:400px;overflow-x:auto;">
                <el-form ref="updata" :model="updata" label-width="70px">
                    ...
                    <el-form-item label="封面图" style="margin-top:130px;">
...
export default {
    ...
    created(){
       this.getProductList();//初始渲染列表调用
    },
    methods: {
        getProductList() {//初始渲染列表方法
            let str='';
             request.post("/api/article1").then(res => {
                if (res.code === 200) {
        ...
        deleteList(row) {//删除商品
            ...
            this.$confirm("您确定要删除？", "提示", {
                ...
                request.post("/api/article1/delete", {
                        id:row.id
                    }).then(res => {
                        this.getProductList()
        ...
        confirmRevision(){//点击确定修改
            ...
            // this.picture_all=JSON.stringify(this.picture_all)
            
            request.post("/api/article1/edit",{//发送数据到后台
                //后台参数
                id : this.updataId,
