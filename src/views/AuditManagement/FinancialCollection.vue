    ...
    <el-table 
      ...
      width="300px" align="center">
        <template slot-scope="scope">
          ...
          <el-button size="small" type="primary" v-if="activeName == 'first'" @click="AssignTasks(scope.row)" >审核提交</el-button>
    ...
    methods: {
      handleClick(tab, event){//改变状态
        ...
        if(this.activeName == 'first'){
          ...
          // console.log('2')
          request.post("/admin/Auditing/financeInquire").then(res => {
            if (res.code == 200) {
      ...
      getAgentList() {//初始渲染列表方法封装某人
        request.post("/admin/Auditing/financeQuery").then(res => {
            if (res.code == 200) {
              this.agentList = res.data.list;
              this.count = res.data.page.count;
              this.max = res.data.page.max;
      ...
      if(this.activeName == 'first'){
          request.post("/admin/Auditing/financeQuery",{
          keyword : this.search,
          ...
          request.post("/admin/Auditing/financeInquire",{
          keyword : this.search,
          // page : this.currentPage,
          }).then(res => {
              if (res.code == 200) {
      ...
      AssignTasks(row){//分配任务
        this.$confirm("审核提交确定？", "提示", {
                confirmButtonText: "确定",
                cancelButtonText: "取消"
            }).then(() => {
                request.post("/admin/Auditing/financeAffirm", {
                        id:row.id,
                        finance_status : '1',
