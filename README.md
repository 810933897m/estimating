...
└── views -- 前端页面
    ├── home -- 首页
    ├── login -- 登录页
    ├── inquiryManagement -- 询价立项管理 inquiry:询价 projectIntiation:报告立项
    ├── fieldManagement -- 外业管理
    ├── AuditManagement --审核管理
    ├── 
    ├── productPolicy -- 产品政策管理
    ├── productManagement -- 产品管理
    ├── classification -- 分类管理
    ├── learningVideo -- 学习视频管理
    ├── distribution -- 分销管理
    ├── userAgreementList --用户协议
    ├── incentiveplan --激励计划与级别认证
    └── 
```
